...
DECAY   1000022     1.00000000E-1   # neutralino1 decays
    0.50000000E+00   3    2   1   5   # n1 -> u d b
    0.50000000E+00   3   -2  -1  -5   # n1 -> ~u ~d ~b
...
model = "RChiWH"
print "pp -> C1 N2, C1 -> W N1, N2 -> H N1"
print "W -> any, H -> any, N1 -> udb"
...
minDM = 140 # min DeltaM between chi and lsp (use 140 GeV to keep H on-shell)
chi_MassList = [200,250,300,350,400,450,500,550,600,
                650,700,750,800,850,900,950,1000,1050,1100,1150,1200]
lsp_MassList = [25,50,75,100,125,150,175,200,250,300,350,400,450,500,550,600]

cols = []
for mx in chi_MassList:
  col = []
  for my in lsp_MassList:
    if ((mx-my)>=minDM):
      if (my > 0):
        col.append([mx,my])
  cols.append(col)
